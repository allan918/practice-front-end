...
extractOpponent(huskyGames2016[1]);
...
var opponent = (huskyGames2016.map(extractOpponent));
...
var lostOppo = huskyGames2016.filter(huskiesLost).map(extractOpponent);
console.log(lostOppo);
...
gameLost.forEach(function (element) {
  if (element.home == "UW") {
    console.log(extractOpponent(element) + " at UW, " + element.opponent_score + " to " + element.home_score);
  } else {
    console.log("UW at " + extractOpponent(element) + " , " + element.home_score + " to " + element.opponent_score);
  }
});
...
var includeFum = huskyGames2016.filter(function (element){
  return element.fumbles >= 1;
});

console.log(includeFum.length);
...
var mostPassingGame = huskyGames2016.reduce(function (element1, element2){
  return mostYardsPassing(element1,element2);
});
...
function makeCombinedFilter(f1,f2) {
  var combinedFilter = function (a) {
    return a.filter(f1).filter(f2);
  };
...
var fumbledAndLostFilter = makeCombinedFilter(huskiesLost, function (element){
  return element.fumbles >= 1;});
...
var lostWithFum = fumbledAndLostFilter(huskyGames2016);
console.log(lostWithFum);
