...
first.addEventListener('click',function(){
    if (task.complete) {
      task.complete = false;
      first.classList.remove("font-strike");
    } else {
      task.complete = true;
      first.classList.add("font-strike");
    }
});
...
first.innerHTML = task.description;
return first;
}
console.log(renderTaskItem(state.taskList[0]).addEventListener);


/*var first = document.querySelectorAll("li");
first.addEventListener("Click",function() {
  for(var i = 0; i < state.taskList; i++) {
    if (state.taskList[i].complete) {
      state.taskList[i].complete = false;
    } else {
      state.taskList[i].complete = true;
    }
  }
});*/
...
function renderTaskList() {
  ...
  for (var i = 0; i < state.taskList.length; i++) {
    ol.appendChild(renderTaskItem(state.taskList[i]));
  }
  renderInput();
}
renderTaskList();
