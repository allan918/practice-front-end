...
export class PetCard extends Component {
    render() {
        ...
        return(<div className="card" onClick={()=>this.props.adoptCallback(card.name)}>
            <img className="card-img-top" src={card.img} alt={card.name} />
            <div className="card-body">
                <h3 className="card-title">{card.adopted?card.name + " (Adopted)":card.name} </h3>
                <p className="card-text">{card.sex + " " + card.breed}</p>
            </div>
        </div>)
    }
}
...
export class PetList extends Component {
    render() {
        let card = this.props.pets;
        console.log(card);
        let cardList = card.map(
